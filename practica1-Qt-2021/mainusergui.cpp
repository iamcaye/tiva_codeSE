...
#define RIGHT_BUTTON			0x00000001
#define LEFT_BUTTON				0x00000010
...
void MainUserGUI::on_colorWheel_colorChanged(const QColor &arg1)
{
    ...
    arg1.getRgb(&colores[0],&colores[1],&colores[2], NULL);
    tiva.sendMessage(MESSAGE_RGB_COLOR, QByteArray::fromRawData((char *)&colores, sizeof(colores)));
...
void MainUserGUI::messageReceived(uint8_t message_type, QByteArray datos)
{
    ...
    switch(message_type) // Segun el comando tengo que hacer cosas distintas
    {
        ...
        {
            uint32_t status = 0;
            if(check_and_extract_command_param(datos.data(), datos.size(), &status, sizeof(status)) > 0)
            {
                if(!(status & LEFT_BUTTON))
                {
                    ui -> left_button_led -> setChecked(true);
                    ui -> statusLabel -> setText(tr("Left button"));
                }
                else
                {
                    ui -> left_button_led -> setChecked(false);
                }

                if(!(status & RIGHT_BUTTON))
                {
                    ui -> statusLabel -> setText(tr("Right button"));
                    ui -> right_button_led -> setChecked(true);
                }
                else
                {
                    ui -> right_button_led -> setChecked(false);
                }
            }
            break;
        }
        case MESSAGE_BUTTONS_SONDEO:
        {
            if(check_and_extract_command_param(datos.data(), datos.size(), &status, sizeof(status)) > 0)
            {
                if(!(status & LEFT_BUTTON))
                {
                    ui -> sondeo_left-> setChecked(true);
                    ui -> statusLabel -> setText(tr("Left button"));
                }
                else
                {
                    ui -> sondeo_left -> setChecked(false);
                }

                if(!(status & RIGHT_BUTTON))
                {
                    ui -> statusLabel -> setText(tr("Right button"));
                    ui -> sondeo_right-> setChecked(true);
                }
                else
                {
                    ui -> sondeo_right-> setChecked(false);
                }
            }
            break;
        }
...
void MainUserGUI::on_SondeoBTN_clicked()
{
    tiva.sendMessage(MESSAGE_BUTTONS_SONDEO,NULL,0);
}


void MainUserGUI::on_activaSondeo_toggled(bool checked)
{
    tiva.sendMessage(MESSAGE_BUTTONS, (void *)&checked, sizeof(checked));
}
